-- Report: 3
-- Title: Deuda por Contrato SOLO INACTIVOS
-- TableName: Contract
-- Category: 113
-- Source: ia-assets/pg-migration/26.07.18_Reportes.txt (fila IdReport=3)
-- Notes: Variante de Report 1 filtrada a contratos inactivos (`and c."IdTermContractStatus" = 6`
--   inmediatamente despues de {WHERE1}, dentro del inner). Sin columnas cAction/cActionDate ni el
--   join `Terms t` (no aplican a este reporte). suspensionDateTime usa solo IdTermTicketAction=48.
--   Mismas reglas de traduccion que Report 1 (ISNULL->COALESCE, IIF->CASE, FORMAT->to_char,
--   JSON_VALUE seguro, FOR XML PATH->string_agg, CHARINDEX->strpos, year/month->EXTRACT, Pay=0->false,
--   identificadores con case exacto del DDL).
