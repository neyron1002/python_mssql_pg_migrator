-- Report: 8
-- Title: Support Ticket Ejecutado Detallado
-- TableName: SupporTicket
-- Category: 113
-- Source: ia-assets/pg-migration/26.07.18_Reportes.txt (fila IdReport=8)
-- Notes: Lote T-SQL multi-#temp reescrito a un unico statement con WITH (CTEs)
--   por la LEY CRITICA de CONVENTIONS (c): Npgsql protocolo extendido = una sola
--   sentencia. hashAudit (HASHBYTES SHA2_256) traducido a lower(encode(digest(...,
--   'sha256'),'hex')) via pgcrypto (00_init crea la extension); es un valor calculado
--   pero solo lo consumia el bloque de auditoria contra Geus_ISP_Audit_DB, que aqui
--   queda como comentario /* */ (JOIN inter-base irreducible en una sola conexion; el
--   original ya lo tenia comentado). Se conserva la 2a ocurrencia de {WHERE1} dentro
--   de ese comentario para respetar where1Count=2. solutionTime: cadena legible
--   "<dias> d HH h MI m SS s" reconstruida por aritmetica de segundos (equivalente a
--   los DATEADD/DATEDIFF/STUFF/CONVERT 114 del T-SQL). FORMAT(f,'yyyy-MM-dd hh:mm tt')
--   -> to_char(f,'YYYY-MM-DD HH12:MI AM'). Parser de direccion (CHARINDEX 3-arg) ->
--   POSITION sobre substring con offset. {WHERE2} filtra "Date" (DateRange).
